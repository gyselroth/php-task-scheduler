...
| `interval_reference`  | `end`  | string | You may specify if the interval refers to the `start` or the `end` of the previous job. The default is `end` which means the interval refers to the end of the previous job. When you define `start` the interval refers to the start of the previous job. This may be useful when a job has to run at specific times. |
